...
Компилятор gcc ARM поставляется вместе с библиотекой newlib, которая реализует
libc.
...
передаст ему необоходимые опции для подключения стандартной библиотеки.
...
использование libc придётся заплатить в любом случае.
...
При линковке мы опять столкнулись с теми же ошибками:
...
их реализацией будет служит обычный вечный цикл. Если вдруг мы туда попадём (это
можно выяснить с помощью отладчика), значит всё же реализация нужна. Реализацию
мы поместим в отдельный файл `os.c`.
...
Эти функции уже вызываются при вызове функции `printf` и вечным циклом здесь
обойтись не выдет.
...
0x20000000 - начало секции .data
...        - содержимое секции .data
0x200006b8 - конец секции .data (размер не обязательно такой)
0x200006b8 - начало секции .bss
...        - содержимое секции .bss
0x200009E8 - конец секции .bss
...        - неиспользуемая память
0x20004900 - минимальное значение регистра $sp при максимальном размере стека
0x20005000 - конец SRAM
```

Как видно, "снизу" расположены секции `.data` и `.bss`, "сверху" расположен
стек, а между ними расположена неиспользуемая память. Вот эту неиспользуемую
память мы и будем возвращать из `sbrk`. В нашем коде никаких проверок не будет,
но вообще говоря такие проверки стоило бы добавить, чтобы куча и стек не
пересеклись.
