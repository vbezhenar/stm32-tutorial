...
Вообще говоря, прошивка уже была описана в предыдущем пункте. Нам нужно создать
такой файл, в котором будет записано некое число, которое мы присвоим регистру
sp в первых 4 байтах, далее там будет записано, к примеру, `0x08000131` в
следующих четырёх байтах, потом будет 296 нулей, а потом будет 2 инструкции по 4
байта, которые будут что-то делать. Итого файл прошивки должен занимать 4 + 4 +
296 + 4 + 4 = 312 байтов. Этот файл мы прошьём по адресу 0x08000000, где и
располагается флеш-память.
...
значение, которое мы присвоим регистру `sp`. В принципе по выражению очевидно,
что мы ему присваиваем значение адреса сразу за окончанием адресного
...
выставленный единичный бит, поэтому мы его и выставляем.
...
`code` и копирует её в данном месте. Кроме того линкер делает то, ради чего его,
собственно, и используют. Он понимает, что `reset_exception_handler` уже равен
не `0`, а `0x0800_0130` и в нужном месте будет записан правильный адрес. Если у
нас есть несколько функций в разных файлах, которые вызывают друг друга, то
линкер разберётся, у какой функции какой итоговый адрес и правильно всё
скомпонует. Если вы видели в других линкер скриптах выражение вроде `*(.text)`,
то это примерно то же: `*` это все файлы, `.text` это название секции, которую
принято использовать для кода. Но в данном примере всё указано максимально явно
и для наглядности использовано нестандартное название секции.
...
адреса. А вот то, что начинается с адреса `0x0800 0130` это уже самый, что ни на
...
Пора заливать эту прошивку в микроконтролер:
...
Собственно: всё. Прошивка залита в микроконтролер, он перезагрузился и теперь
...
устанавливать `sp` именно в конце, можно ему выделить любой удобный участок
оперативной памяти, но в простых программах разумно стеку отдать верхнюю часть
памяти, с большими адресами, а свои переменные располагать в нижней части
памяти, с меньшими адресами.

В нашей программе стек не используется, поэтому в принципе туда можно писать
любое значение. Но надо понимать, что если стек используется, а мы в `sp`, к
примеру, записали 0, то при попытке записать что-нибудь в стек, процессор
попытается записать значение по адресу `0xffff_fffc` и зависнет (точней сначала
попытается вызвать обработчик bus fault, т.к. его нет, попытается вызвать
обработчик hard fault, его тоже нет и он зависнет).

Во-вторых откуда взялось число 0x130, почему бы нам не расположить наш код сразу
же со смещением 8. На самом деле это можно сделать и всё будет работать в данном
конкретном случае. Но в общем случае так делать не нужно. В начале адресного
пространства расположена т.н. таблица векторов (название странное, не ищите в
нём смысл). Правильней её назвать таблицей указателей на обработчики исключений.
Сразу скажу, что это не исключения из C++, это исключения процессора. Некоторые
исключения вызываются прерываниями, некоторые исключения вызываются по другим
причинам. К примеру если вы попробуете скормить процессору какую-нибудь дичь, то
вызовется исключение под названием usage fault. Когда процессор вызывает
исключение, он приостанавливает текущий код (который, кстати, может обрабатывать
другое исключение), находит адрес обработчика исключений в таблице векторов,
проверяет, что у этого адреса младший бит выставлен в единицу и вызывает функцию
с этим адресом.
